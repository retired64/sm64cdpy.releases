...
//   3. JSON bundled      → assets/db/vip.json  (fallback).
//
...
class VipModDatasource {
  List<VipModModel>? _cache;

  // ── URLs y nombres de archivo ──────────────────────────────────────────────

  static const String _remoteUrl = AppConstants.vipModsRemoteUrl;
  static const String _localFileName = 'vip.json';
  ...
  Future<List<VipModModel>> getAll() async {
  ...
  Future<String> _readRaw() async {
    ...
    return rootBundle.loadString(AppConstants.vipModsAssetPath);
  }
  ...
  Future<FetchResult> fetchRemote() async {
    try {
      ...
      // Validación mínima: JSON válido con clave "vip_mods"
      final decoded = json.decode(body) as Map<String, dynamic>;
      if (!decoded.containsKey('vip_mods')) {
        return const FetchResult.error('Invalid database format received.');
      }

      final modCount = (decoded['vip_mods'] as Map<String, dynamic>).length;
  ...
  List<VipModModel> _parse(String raw) {
    final data = json.decode(raw) as Map<String, dynamic>;
    final modsMap = data['vip_mods'] as Map<String, dynamic>? ?? {};
    return modsMap.entries.map((e) {
      final map = e.value as Map<String, dynamic>;
      final id = map['id'] as String? ?? e.key;
      return VipModModel.fromJson(id, map);
    }).toList();
  }
