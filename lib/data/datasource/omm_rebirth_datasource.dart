...
//   3. JSON bundled      → assets/db/omm.json  (fallback).
//
...
class OmmRebirthDatasource {
  List<OmmRebirthModel>? _cache;

  // ── URLs y nombres de archivo ──────────────────────────────────────────────

  static const String _remoteUrl = AppConstants.ommRebirthRemoteUrl;
  static const String _localFileName = 'omm.json';
  ...
  Future<List<OmmRebirthModel>> getAll() async {
  ...
  Future<String> _readRaw() async {
    ...
    return rootBundle.loadString(AppConstants.ommRebirthAssetPath);
  }
  ...
  Future<FetchResult> fetchRemote() async {
    try {
      ...
      // Validación mínima: JSON válido con clave "omm_mods"
      final decoded = json.decode(body) as Map<String, dynamic>;
      if (!decoded.containsKey('omm_mods')) {
        return const FetchResult.error('Invalid database format received.');
      }

      final modCount = (decoded['omm_mods'] as Map<String, dynamic>).length;
  ...
  List<OmmRebirthModel> _parse(String raw) {
    final data = json.decode(raw) as Map<String, dynamic>;
    final modsMap = data['omm_mods'] as Map<String, dynamic>? ?? {};
    return modsMap.entries.map((e) {
      final map = e.value as Map<String, dynamic>;
      final id = map['id'] as String? ?? e.key;
      return OmmRebirthModel.fromJson(id, map);
    }).toList();
  }
