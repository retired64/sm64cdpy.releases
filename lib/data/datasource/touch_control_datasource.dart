...
//   3. JSON bundled      → assets/db/touch_controls.json  (fallback).
//
...
class TouchControlDatasource {
  List<TouchControlModel>? _cache;

  // ── URLs y nombres de archivo ──────────────────────────────────────────────

  static const String _remoteUrl = AppConstants.touchControlsRemoteUrl;
  static const String _localFileName = 'touch_controls.json';
  ...
  Future<List<TouchControlModel>> getAll() async {
  ...
  Future<String> _readRaw() async {
    ...
    return rootBundle.loadString(AppConstants.touchControlsAssetPath);
  }
  ...
  Future<FetchResult> fetchRemote() async {
    try {
      ...
      // Validación mínima: JSON válido con clave "touch_controls"
      final decoded = json.decode(body) as Map<String, dynamic>;
      if (!decoded.containsKey('touch_controls')) {
        return const FetchResult.error('Invalid database format received.');
      }

      final modCount =
          (decoded['touch_controls'] as Map<String, dynamic>).length;
  ...
  List<TouchControlModel> _parse(String raw) {
    final data = json.decode(raw) as Map<String, dynamic>;
    final modsMap = data['touch_controls'] as Map<String, dynamic>? ?? {};
    return modsMap.entries.map((e) {
      final map = e.value as Map<String, dynamic>;
      final id = map['id'] as String? ?? e.key;
      return TouchControlModel.fromJson(id, map);
    }).toList();
  }
